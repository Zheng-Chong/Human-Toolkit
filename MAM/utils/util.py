...
import logging
import numpy as np
from MAM.utils.config import CONFIG
...
def load_imagenet_pretrain(model, checkpoint_file):
    ...
    """
    checkpoint = torch.load(checkpoint_file, map_location = lambda storage, loc: storage.cuda(CONFIG.gpu), weights_only=False)
...
def load_VGG_pretrain(model, checkpoint_file):
    ...
    Since we have the spectral normalization, we need to do a little more
    """
    checkpoint = torch.load(checkpoint_file, map_location = lambda storage, loc: storage.cuda(), weights_only=False)
